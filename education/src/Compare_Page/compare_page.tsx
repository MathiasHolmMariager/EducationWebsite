...
function ComparePage() {
    ...
    const favoritesKandidat = uddannelseData.filter(item => favoritesStudy.includes(item.title) && item.type === "master" ).flatMap(index => index.title);
    const favoritesBachelor = uddannelseData.filter(item => favoritesStudy.includes(item.title) && item.type === "bachelor" ).flatMap(index => index.title);
    const [optionSelect, setOptionSelect] = useState('');
    const barColors = ['#09214C', '#FAA712', '#FBD607', '#DBDE4F', '#4FA169'];
    const lønData = uddannelseData.filter(item => favoritesStudy.includes(item.title) && item.type === "master" ).flatMap(item => item.FørsteÅrLøn).map((FørsteÅrLøn, index) => ({ ...FørsteÅrLøn, fill: barColors[index % barColors.length] }));
    const lønTiData = uddannelseData.filter(item => favoritesStudy.includes(item.title) && item.type === "master" ).flatMap(item => item.TiÅrLøn).map((TiÅrLøn, index) => ({ ...TiÅrLøn, fill: barColors[index % barColors.length] }));
